...
async def list_menu(page: int = 0) -> InlineKeyboardMarkup:
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton(text="⬅️ Назад", callback_data=f"menu_page_{page - 1}")
        )
    if end_idx < len(cakes):
        nav_buttons.append(
            InlineKeyboardButton(text="Вперед ➡️", callback_data=f"menu_page_{page + 1}")
        )
...
async def view_cake(cake_index: int) -> InlineKeyboardMarkup:
    ...
    order_button = InlineKeyboardButton(
        text="✅ Оформить", callback_data=f"order_cake_{cake_index}"
    )
    # Кнопка возврата к меню
    back_to_menu_button = InlineKeyboardButton(
        text="🍰 Назад к меню",
        callback_data="list_menu",
    )
...
async def order_status(status: bool, order_id: int) -> InlineKeyboardMarkup:
    ...
    button_paid = InlineKeyboardButton(
        text=f"{paid_emoji} Оплачено", callback_data=f"order_paid_{order_id}"
    )
    button_unpaid = InlineKeyboardButton(
        text=f"{unpaid_emoji} Не оплачено", callback_data=f"order_unpaid_{order_id}"
    )
